...
"""),
    ...
    if raw_output is None:
        #แทนที่จะ raise error
        return ClarificationResult(
            clarification_needed=True,
            reason="ระบบไม่สามารถประมวลผลคำถามได้ โปรดลองใหม่ค่ะ"
        )
...
def run_classification_pipeline(state: AgentState) -> AgentState:
    ...
    print(f"should_terminate? {state.should_terminate()}")

    if state.should_terminate():
        print("TERMINATE after Clarity")
        state.response = (
            f"รบกวนขอข้อมูลเพิ่มเติม: {state.classification_result.clarification_reason}"
        )
    ...
    cats = [c.category for c in state.classification_result.category_level_1]
    subs = [s.subcategory for s in state.classification_result.category_level_2]
    state.response = (
        "หมวดหมู่ที่ตรวจพบ:\n"
        f"- หมวดหลัก: {', '.join(cats) if cats else 'ไม่พบ'}\n"
        f"- หมวดย่อย: {', '.join(subs) if subs else 'ไม่พบ'}"
    )
    state.terminated = True

    print("RESPONSE SET:", state.response)
    return state
