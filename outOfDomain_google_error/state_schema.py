...
class ClassificationResult(BaseModel):
    category_level_1: Optional[List[CategoryScore]] = Field(
        None, description="หมวดหมู่หลักพร้อม confidence"
    )
    category_level_2: Optional[List[SubcategoryScore]] = Field(
        None, description="หมวดย่อยพร้อม confidence"
    )
    ...
    intent: Optional[str] = Field(
        None, description="Intent พิเศษ เช่น greeting, farewell, retrieval"
    )
